...
PADと連動した自動アップロードの中核機能。WebSocketによるリアルタイム進捗表示、キャンセル処理、エラーハンドリングを含む。**v3.2.0よりドキュメント削除機能とマスターテーブル管理機能を追加**。**v3.5.0より患者別表示の削除ボタン、キュータスク個別削除機能、モーダルz-index管理を追加**。**v4.1.0よりready_to_printステータス機能による印刷ワークフローを実装**。

#### [AI報告書作成機能](./specifications/02-ai-report-v4.md)
カルテから居宅療養管理指導報告書を自動生成。2ステップのシンプルなフローで、Azure OpenAI (gpt-4o-mini)を使用した高精度な情報抽出。**v4.0.0よりAzure OpenAI APIへ完全移行**。LocalStorage専用設計、インライン編集機能、カルテ貼り付けタブ機能を含む。
...
#### [PDF連結印刷機能](./specifications/09-batch-print.md)
ready_to_printステータスのPDFを選択・連結して一括印刷する機能。**v5.0.2実装済み**：サーバーサイドPDF処理、最大200件対応、ソート機能付き選択UI、mergingステータス対応、完了モーダル表示。
...
Power Automate Desktopとの連携詳細。キュー管理、自律処理フロー、トラブルシューティングガイド。**v4.1.0より新ステータスフロー（pending → processing → uploaded → ready_to_print → done）を実装**。
...
開発環境から運用環境への完全移行手順。データベースバックアップ・リストア、ファイルシステム移行、環境設定の詳細。

#### [ネットワーク設定ガイド](./local_network_setup_guide.md)
ローカルネットワーク内の複数PCからアクセス可能にするための設定。PostgreSQL、Node.js、ファイアウォールの設定手順。

#### [移行作業記録](./migration_summary_20250925.md)
2025年9月25日実施の移行作業の詳細記録。作成したバックアップ、スクリプト、ドキュメントの一覧。
...
### Phase 1.15（v4.1.0 - 2025年9月21日完了）✅
- **ready_to_printステータス機能**: 印刷ワークフロー管理の実装
  - 新ステータスフロー: pending → processing → uploaded → ready_to_print → done
  - PADアップロード完了後、自動的にready_to_printステータスへ遷移
  - トリガーによるDocuments更新とファイル移動の自動化
  - フロントエンドでの新ステータス表示（印刷待ち 🖨️）
- **キュー削除機能の強化**:
  - 個別キューアイテムの削除機能（DELETE /api/queue/:id）
  - キューモニターでの削除操作UI
  - 削除確認モーダルによる誤操作防止
  - WebSocket通知による即座のUI更新
- **PAD API更新**:
  - `/api/queue/:id/uploaded` エンドポイント追加（アップロード完了用）
  - `/api/queue/:id/ready-to-print` エンドポイント追加（印刷準備完了用）
  - `/api/queue/:id/done` エンドポイント追加（印刷完了用）
  - 後方互換性維持（`/complete` → `/uploaded` リダイレクト）

### Phase 1.16（v5.0.0～5.0.2 - 2025年9月22日～23日）✅
- **PDF連結印刷機能**: ready_to_printステータスの一括印刷ワークフロー
  - **サーバーサイドPDF連結処理**（pdf-lib使用）✅
  - **最大200件まで選択可能**（メモリ最適化：500MB制限）✅
  - **非同期処理**（Redis不要のシンプルジョブキュー実装）✅
  - **ソート機能**（患者名/ファイル名/作成日時）✅
  - **選択式UI**（デフォルト全選択、上限200件）✅
  - **エラー処理**:
    - 破損PDFの自動スキップ（mergingステータス維持）✅
    - パストラバーサル攻撃対策✅
    - documentIds検証機能✅
  - **ステータス管理**:
  ...
  - **データ管理**:
    ...
    - 60日経過アラート（通知のみ、自動削除なし）✅
  - **制約事項（設計通り）**:
    - プロセス再起動時、処理中ジョブは失われる
    - 同時実行は1ジョブのみ（順次処理）
    - エラー時は手動再実行が必要
  - **v5.0.1修正（2025年9月23日）**:
    - WebSocketリスナー登録問題を修正（window.App代入追加）
    - 進捗表示のDOM更新処理を追加
    - WebSocket未接続時の再試行ロジック実装
  - **v5.0.2改善（2025年9月23日）**:
    - 完了通知をモーダル表示に変更
    - "印刷か保存を行ってください"メッセージ追加
    - 失敗件数の橙色警告表示

### Phase 1.17（v5.1.0 - 2025年9月23日完了）✅
- **連結PDF履歴管理機能**: batch_printsテーブルのUI管理機能実装
  - **履歴モーダル表示機能**（batchPrintHistory.js）:
    - キューモニター内の「連結履歴」ボタンから起動
    - 作成日時順でPDF一覧を表形式で表示
    - ファイル名、サイズ、ページ数、文書数、作成日時を表示
  - **履歴API実装**（batchPrintHistoryApi.js）:
    - GET /api/batch-print/history - 履歴取得
    - DELETE /api/batch-print/:batchId - PDF削除
    - /api/batch-print/view/:batchId - PDF表示
  - **PDF表示機能**:
    - 「表示」ボタンで新しいタブにPDFを開く
    - ポップアップブロッカー対策実装
  - **削除機能**:
    - 削除確認ダイアログ（App.jsの共通確認モーダルを使用）
    - 物理ファイルとDBレコードの同時削除
    - 削除後の即座のリスト更新
  - **60日経過アラート**:
    - 60日以上経過したPDFを黄色背景で警告表示
    - アプリ起動時にトースト通知（セッションストレージで1回限り）
    - 削除ボタンを赤色の危険ボタンスタイルに変更
  - **UIデザイン統一**:
    - 既存のモーダル管理システムとの完全統合
    - z-index管理によるモーダルスタッキング対応
    - レスポンシブテーブルレイアウト
...
*バージョン: 5.1.0（実装完了）*
 ...
 完了した作業のまとめ
  ...
  1. データベースバックアップ作成完了
    - 場所: backup_20250925_091032/ageagekun.dump
    - サイズ: 約8.7 MB
  2. ファイルバックアップ作成完了
    - patientsフォルダ: 約3.6 MB
    - 環境設定ファイル: .env.backend, .env.local

  📁 作成したドキュメント・スクリプト

  1. docs/migration_guide.md - 完全な移行手順書
  2. docs/local_network_setup_guide.md - ネットワーク構成ガイド（データベ 
ース移行セクション追加済み）
  3. backup_dev.ps1 - 自動バックアップスクリプト
  4. .env.production.template - 運用環境用設定テンプレート
  5. test_production.ps1 - 動作確認テストスクリプト
  6. backup_20250925_091032/README_BACKUP.md - バックアップ情報

  📦 バックアップフォルダ内容

  backup_20250925_091032/
  ├── ageagekun.dump          # PostgreSQLデータベース
  ├── patients/                # 患者ファイル
  ...
  このバックアップを運用PCに転送して、以下の手順でリストアしてください：  

  1. バックアップフォルダ全体をUSBまたはネットワーク経由で転送
  2. 運用PCでPostgreSQL 17とNode.jsをインストール
  3. GitHubからソースコードをクローン
  4. docs/migration_guide.mdの手順に従ってリストア
  5. test_production.ps1で動作確認

  すべての作業が完了しました。これで開発環境から運用環境への移行準備が整い
ました。

> この内容もどこかにドキュメントとしてまとめておいてくｄさい\
  移行手順書などに\
  \
  そしてドキュメントが散逸しているのでお互いに参照できるようにしておいてく
ださい\
  またclaude.mdからも参照できるようにしておいてください\
  \
  ドキュメンテーションしてほしい内容は下記\
  ...
  ● 完了した作業のまとめ
    ...
    1. docs/migration_guide.md - 完全な移行手順書
    2. docs/local_network_setup_guide.md - ネットワーク構成ガイド（データ 
ベース移行セクション追加済み）
    3. backup_dev.ps1 - 自動バックアップスクリプト
    4. .env.production.template - 運用環境用設定テンプレート
    5. test_production.ps1 - 動作確認テストスクリプト
